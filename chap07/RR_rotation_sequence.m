...
% SOME TESTS:
%   a=rand(3,1)                                  % Define a random 3D initial vector to be rotated
%   r=RR_rotation_sequence([3 2 1],90*rand(1,3)) % Define a 321 Tait-Bryan rotation with random angles
%   b=rotate(a,r)                                % Rotate a using the rotation sequence r
%   % A 123 Tait-Bryan rotation with opposite angles as r
%   r2=RR_rotation_sequence([1 2 3],-[r.an(3) r.an(2) r.an(1)])
%   a1=rotate(b,r2)                              % Using r2, rotate b back to the direction of a
...
classdef RR_rotation_sequence < matlab.mixin.CustomDisplay
    ...
    methods
        ...
        function a = rotate(a,r)         % Rotate a 3D vector a by any rotation sequence r
            a=rotation_matrix(r)*a;
        end
        function q = quaternion(r)       % Convert any rotation sequence r to a corresponding quaternion q
        ...
        function R = rotation_matrix(r)  % Convert any rotation sequence r to a corresponding rotation_matrix R    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    methods(Access = protected)
        function displayScalarObject(r)
            fprintf(getHeader(r))
            fprintf('%g %s rotation sequence with [alpha,beta,gamma] = %g %g %g\n', ...
                 r.ax(1)*100+r.ax(2)*10+r.ax(3),r.t,r.an)
        end
    end
end
