...
% function h = RR_calculate_h(v,k,r,verbose)
...
% NOTE 2: This simple demo code requires k+r<=63.
% EXAMPLE CALL [computes h(z) for v(z) corresponding to CRC-5-USB, for k=26, d=3]:
%     k=26; r=5; v=0b100101u64; h=RR_calculate_h(v,k,r,true);
% EXAMPLE CALL [computes h(z) for v(z) corresponding to CRC-5-ITU, for k=10, d=4]:
%     k=10; r=5; v=0b101011u64; h=RR_calculate_h(v,k,r,true);
% Renaissance Robotics codebase, Chapter 7, https://github.com/tbewley/RR
% Copyright 2021 by Thomas Bewley, distributed under BSD 3-Clause License.

if ~verbose
  t=bitshift(1,k+r)+0b1u64; h=0b0u64;  % t = 100...001
  for  i=k+r+1:-1:r+1                  % h = t / v on F2 
    if bitget(t,i)
      h=h+bitshift(1,i-r-1); t=bitxor(t,bitshift(v,i-r-1));        
    end
  end
else
  t=bitshift(1,k+r)+0b1u64; h=0b0u64;  ts=dec2bin(t)                            
  for  i=k+r+1:-1:r+1                  % (same as above, with more screen output)
    if bitget(t,i)
      w=bitshift(v,i-r-1);   vs=dec2bin(w,k+r+1)
      h=h+bitshift(1,i-r-1); hs=dec2bin(h)
      t=bitxor(t,w);         ts=dec2bin(t,k+r+1)
    end
  end
  ...
  for  i=1:k+1
    if bitget(h,i)
      w=bitshift(v,i-1); vs=dec2bin(w,k+r+1)
      c=bitxor(c,w);     check=dec2bin(c,k+r+1)
    end
  end
end