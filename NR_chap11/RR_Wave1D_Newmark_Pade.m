function [q,x]=RR_Wave1D_Newmark_Pade(L,Tmax,c,N,dt)
% function [q,x]=RR_Wave1D_Newmark_Pade(L,Tmax,c,N,dt)
...
% See also RR_Wave1D_ItCN_FD, RR_Wave1D_Newmark_Pade.

dx=L/N; t=0; x=(-N/2:N/2-1)'*dx; q=exp(-x.^2/0.1); v=0; RR_PlotXY(x,q,t,-L/2,L/2,-0.2,1.2);
...
a=RR_ThomasTT(0.1,1,.1,dd*q([N 1:N-1],1)+ee*q([1:N],1)+dd*q([2:N 1],1),N);
for n=1:Tmax/dt
  as=q+dt*v+dt2*b1*a;
  as=RR_ThomasTT(aa,bb,aa,-dd*as([N 1:N-1],1)-ee*as([1:N],1)-dd*as([2:N 1],1),N);
  q=q+dt*v+dt2*(b1*a+b2*as); v=v+dt*(g1*a+gamma*as); a=as;
  t=t+dt; RR_PlotXY(x,q,t,-L/2,L/2,-0.2,1.2);
end
end % function RR_Wave1D_Newmark_Pade
