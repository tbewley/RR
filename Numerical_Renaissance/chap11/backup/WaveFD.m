function RR_WaveFD                                     % Numerical Renaissance Codebase 1.0
...
rhs=zeros(N,1); [rhs,a,b,c,d,e]=RR_Circulant(a,b,c,rhs,N); % Determine LU to reuse during march
for n=1:TimeSteps                                            % Leading-order cost:
  rhs(2:N,1)=phi(2:N)+e*(phi(3:N+1)-2*phi(2:N)+phi(1:N-1));  % ~ ?N
  t=t+dt;  rhs(1)=sin(t);                                    %
  u(1:N)=RR_CirculantLU(rhs,a,b,c,d,e,N);                       % ~ 9N
  PlotXY(x,u,t,-L/2,L/2,-0.2,1.2);
end                                                          % Total: ~ ?N per timestep
end % function RR_WaveFD.m