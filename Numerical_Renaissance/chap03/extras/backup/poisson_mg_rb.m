function RR_poisson_mg_rb
% A 2D Poisson solver on uniform mesh using Multigrid and Red/Black RR_Gauss-Seidel
...
for smooth=1:n1, v=RR_poisson_rb(v,d,g); end                     % n1 iterations of smoothing
tic;
for main=1:150
  o=e; v=RR_poisson_rb(v,d,g); e=max_error(v,d,g,main,2,o,main);

%   o=e; v=RR_poisson_mg(v,d,g,n2,n3); e=max_error(v,d,g,main,plots,o);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function vf = RR_poisson_mg(vf,df,gf,n2,n3)
% Apply Multigrid with Red/Black RR_Gauss-Seidel smoothing to the Poisson equation.
...
% gf.xm, gf.ym, df, vf
for smooth=1:n2, vf=RR_poisson_rb(vf,df,gf); end          % n2 iterations of rb smoothing
...
if gc.nx > 3 & gc.ny > 3 & mod(gf.nx,4)==0 & mod(gf.ny,4)==0 
   vc=RR_poisson_mg(vc,dc,gc,n2,n3);                 % Continue down to an even coarser grid,
else                                              % or
   for loop=1:20, vc=RR_poisson_rb(vc,dc,gc); end    % solve coarsest system (almost exactly)
end
for ic=2:gc.xm       % Prolongation (bilinear interpolation) on the black interior points.
   for jc=2:gc.ym    % Note: the next call to RR_poisson_rb will take care of the red points.
      i=2*(ic-gc.xo)+gf.xo; j=2*(jc-gc.yo)+gf.yo;
...
for smooth=1:n3, vf=RR_poisson_rb(vf,df,gf); end                 % n3 iterations of smoothing
% end function RR_poisson_mg
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function v = RR_poisson_rb(v,d,g);
% Apply Red/Black RR_Gauss-Seidel smoothing, with L derived from the Poisson equation
