function RR_Wave1D_ItCN_Pade(L,Tmax,c,N,dt)
% function RR_Wave1D_ItCN_Pade(L,Tmax,c,N,dt)
...
RR_PlotXY(x,q,t,-L/2,L/2,-0.2,1.2); vs=v; qs=v; a=0.6*dt*c^2/dx^2; b=-1.2*dt*c^2/dx^2;
for n=1:Tmax/dt
  for m=1:IterSteps
    if m==1, qs=q+dt*(1.5*v-0.5*qs)/2; else, qs=q+dt*(vs+v)/2; end
    vs=qs+q; vs=v+RR_ThomasTT(0.1,1,0.1,a*vs([N 1:N-1],1)+b*vs([1:N],1)+a*vs([2:N 1],1),N);
  end
  t=t+dt; q=qs; v=vs; RR_PlotXY(x,q,t,-L/2,L/2,-0.2,1.2);
end
end % function RR_Wave1D_ItCN_Pade
