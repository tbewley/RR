function RR_ResponseSS(A,B,C,D,w,g,Input)
% function RR_ResponseSS(A,B,C,D,w,g,Input)
...
for m=1:M
  ...
  Am=eye(N)-A*h/2; [x(:,2),Amod]=RR_Gauss(Am,Ap*x(:,1)+Bs*(u(:,2)+u(:,1)),N);
  for k=3:K+1, [x(:,k)]=RR_GaussLU(Amod,Ap*x(:,k-1)+Bs*(u(:,k)+u(:,k-1)),N); end
