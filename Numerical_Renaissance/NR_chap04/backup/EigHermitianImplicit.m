function [b] = RR_EigHermitianImplicit(A,n,b,c)
% lam=RR_EigHermitian(A,n) computes the eigenvalues of a Hermitian (or symmetric) matrix A.
% Leveraging the fact that RR_Hessenberg.m returns a tridiagonal matrix, the QR algorithm
...
if nargin==2, A=RR_Hessenberg(A); a=[0; diag(A,-1)]; b=diag(A); c=[diag(A,1); 0];
else; a=A; end;
for step=1:20*n;                                  % Wilkenson shift
   ...
   if abs(a(n))< 1e-12*(abs(b(n))+abs(b(n-1)))                  % If b(n) converged,
     if n>2, b(1:n-1)=RR_EigHermitianImplicit(a(1:n-1),n-1,b(1:n-1),c(1:n-1)); end % deflate
     break
   end
end;  if step==20*n, disp('RR_EigHermitianImplicit did not converge'), else, b=real(b); end
end % function RR_EigHermitianImplicit.m
