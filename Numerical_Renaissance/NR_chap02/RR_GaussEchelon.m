function [A,p,r,v,R] = RR_GaussEchelon(A)
% function [A,p,r,v,R] = RR_GaussEchelon(A)
...
% an extension of RR_Gaussian elimination with partial pivoting.  The matrix A is replaced by
...
for j = 1:n,
   ...
   if amax > 0                                    % If it is nonzero, increment the rank,
      ...
      if r<m                                      % if not in the very last row, then
         ...
         end                                      % then perform the RR_Gauss transformation.
