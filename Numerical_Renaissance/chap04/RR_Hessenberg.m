function [A,V] = RR_Hessenberg(A)
% function [A,V] = RR_Hessenberg(A)
% Pre and post-multiply an nxn matrix A by a sequence of Householder reflections to reduce
% it to upper RR_Hessenberg form T_0, thus computing the unitary similarity transformation
% A=V T_0 V^H.  
...
for i=1:n-2
   [sig,w] = RR_reflect_compute(A(i+1:n,i));  A=RR_reflect(A,sig,w,i+1,n,1,n,'B');
   if nargout>1, V=RR_reflect(V,sig,w,i+1,n,1,n,'R'); end
end
end % function RR_Hessenberg
