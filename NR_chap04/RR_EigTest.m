% script <a href="matlab: RR_EigTest">RR_EigTest</a>
% Test <a href="matlab:help RR_Eig">RR_Eig</a> on random matrices of various structure.
...
disp('Now testing RR_Eig on random matrices of various structure.')
clear; n=10; A_gen=randn(n)+sqrt(-1)*randn(n);  [lam_gen,S_gen]=RR_Eig(A_gen,'g');
eig_general_error=norm(A_gen*S_gen-S_gen*diag(lam_gen))

clear; n=20; A=randn(n)+sqrt(-1)*randn(n); A_her=A*A'; [lam_her,S_her]=RR_Eig(A_her,'h');
eig_hermitian_error=norm(A_her*S_her-S_her*diag(lam_her))

clear; n=20; A_real=randn(n); [lam_real,S_real]=RR_Eig(A_real,'r');
eig_real_error=norm(A_real*S_real-S_real*diag(lam_real)), disp(' ')

% end script RR_EigTest
