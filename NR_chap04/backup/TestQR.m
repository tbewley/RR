...
for J=1:N-1, for I=J+2:M, C(I,J)=0; end, end,    % forced to be upper RR_Hessenberg.
disp('Now testing RR_QRGivensHessenberg');
[Q,R]=RR_QRGivensHessenberg(C); nonorthogonality=norm(Q'*Q-eye(size(Q'*Q))), residual=norm(Q*R-C),
disp('Now testing RR_QRFastGivensHessenberg');
[Q,R]=RR_QRFastGivensHessenberg(C); nonorthogonality=norm(Q'*Q-eye(size(Q'*Q))), residual=norm(Q*R-C),
...
disp('Now running RR_QRFastGivensHessenberg');
tic; [Q,R]=RR_QRFastGivensHessenberg(D); toc,
