function [A] = RR_EigReal(A,n,flag)                     % Numerical Renaissance Codebase 1.0
% lam=RR_EigReal(A,n) computes the eigenvalues of a nonsymmetric real matrix A. After an
% initial RR_Hessenberg decomposition, several double implicitly-shifted QR steps are taken
% using the algorithm.  Note that T is deflated whenever T(m,m) converges.
if nargin==2, A=RR_Hessenberg(A); end;  kp=min(4,n);
if n==2
  ...
  if abs(A(n,n-1))< 1e-12*(abs(A(n,n))+abs(A(n-1,n-1)))          
    if n>2, A(1:n-1,1:n-1)=RR_EigReal(A(1:n-1,1:n-1),n-1,1); end; break
  ...
  if abs(A(n-1,n-2))< 1e-12*(abs(A(n,n))+abs(A(n-1,n-1))+abs(A(n,n-1))+abs(A(n-1,n)))   
    if n>3, A(1:n-2,1:n-2)=RR_EigReal(A(1:n-2,1:n-2),n-2,1); end;
  ...
  for k=1:n-2;  km=min(k+3,n); kn=min(k+4,n);           % Transform the rest of the matrix
    ...
    A(1:kn,k+1:km)=A(1:kn,k+1:km)-(A(1:kn,k+1:km)*w)*(sig*w');  % upper RR_Hessenberg form.
  end
end;  if step==20*n, disp('RR_EigReal did not converge'), end
if nargin==2, A=diag(A); end;     % Extract eigenvalues from main diagonal of result.
end % function RR_EigReal.m
