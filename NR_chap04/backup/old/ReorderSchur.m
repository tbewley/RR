function [U,T]=RC_ReorderSchur(U,T,type,e)                % Numerical Renaissance Codebase 1.0
% This function reorders a RC_Schur decomposition such that the stable eigenvalues appear in
% the first n columns and the unstable eigenvalues appear in the last n columns.  
...
switch type(1)
  case 'l'     % type='lhp' for continuous-time systems (first LHP modes, then RHP)
	while p1>0 % (start from bottom right and work towards upper left)
      ...
      while p1>=1 & real(T(p1,p1))>0;  p1=p1-1; end;
      [U,T]=BlockSwap(U,T,p1,p2,p3,g);
    end
  case 'u'     % type='unitdisk' for discrete-time systems (inside, then outside unit disk)
	while p1>0
      ...
      while p1>=1 & abs(T(p1,p1))>1;   p1=p1-1; end;             
      [U,T]=BlockSwap(U,T,p1,p2,p3,g);
    end
  case 'a'     % type = 'absolute', to order by the absolute value of the real part
    for i=n-1:-1:1, a=i+1; b=n;      % (see RC_InsertionSort.m for details)
      while a<b-1; c=a+floor((b-a)/2); if abs(real(T(c,c))+e)<abs(real(T(i,i))+e), a=c+1; else, b=c-1; end, end
      while a<=b;                      if abs(real(T(i,i))+e)<abs(real(T(b,b))+e), b=b-1; else, a=a+2; end, end
      if b>i, [U,T]=BlockSwap(U,T,i-1,i,b,g); end % Insert record i at the designated point.
    end
end
...
T22=T(p2+1:p3,p2+1:p3); m=p2-p1; p=p3-p2;
...
if m>1                                                     % make new T22 upper triangular
  ...
  [V,temp]=RC_Schur(Q12'*T11*(Q12-Q11*R11*Q22/g));
  Q(:,p+1:p+m)=Q(:,p+1:p+m)*V;                             % Q=Q*[eye_(pxp) 0; 0 V_(mxm)]
end
