function [b] = RR_EigHermitian(A,n,b,c)
...
if nargin==2, T=RR_Hessenberg(A); a=[0; diag(T,-1)]; b=diag(T); c=[diag(T,1); 0];
else; a=A; end;
for step=1:20*n;                                  % Wilkenson shift
   ...
   if abs(a(n))< 1e-12*(abs(b(n))+abs(b(n-1)))                   % If b(n) converged,
     if n>2, b(1:n-1)=RR_EigHermitian(a(1:n-1),n-1,b(1:n-1),c(1:n-1)); end % deflate
     break
   end
end;  if step==20*n, disp('RR_EigHermitian did not converge'), else, b=real(b); end
