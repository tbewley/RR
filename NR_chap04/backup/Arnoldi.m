function [A] = RR_Arnoldi(A,n,p)                        % Numerical Renaissance Codebase 1.0
% This routine implements the Implictly Restarted RR_Arnoldi Method to find the n leading
...
for iter=1:itmax
  ...
  % lower RR_Hessenberg.  Now apply p steps of shifted QR to L, accumulating the effects on Q.
  % These QR steps contaminate only the last p columns of L, which we subsequently discard.
  % The first n columns of L retains their RR_Hessenberg structure, eventually converging to a
  % lower triangular form as the routine iterates, thus approaching the first n columns of
  % a lower RR_Schur decomposition A=Q*L*Q^H.
