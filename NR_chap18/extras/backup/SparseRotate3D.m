function SparseRotate3D    % by Thomas Bewley                     
%%%%%%%%%%%%%%%%% begin user input %%%%%%%%%%%%%%%% 
T=1000; G.thresh=0.0001; G.max=3000;
G.dt=.001; G.dx=0.2; G.d=3; G.sigma=4; G.b=1; G.r=48; G.L=20;
%%%%%%%%%%%%%%%% end of user input %%%%%%%%%%%%%%%% 
t=0; G.Y=eye(G.d,'int16'); [D]=Initialize_D(G); h1=round(G.dx/G.dt);
[D]=Modify_pointset(D,G); Rotate_Plot(D,G), title(sprintf('Time = %0.5g',t))
for timestep=1:T/G.dt, t=t+G.dt; if mod(timestep,h1)==0, [D]=Modify_pointset(D,G); end
  % ------------------------------------ RK4 Time March -------------------------------
  % K=RHS_P(D,D.P(1:D.n)                ,G); Pt (2:D.n,1)=D.P(2:D.n)+G.dt*K(2:D.n)/6;
  % K=RHS_P(D,D.P(1:D.n)+G.dt*K(1:D.n)/2,G); Pt (2:D.n,1)=Pt (2:D.n)+G.dt*K(2:D.n)/3;
  % K=RHS_P(D,D.P(1:D.n)+G.dt*K(1:D.n)/2,G); Pt (2:D.n,1)=Pt (2:D.n)+G.dt*K(2:D.n)/3;
  % K=RHS_P(D,D.P(1:D.n)+G.dt*K(1:D.n)  ,G); D.P(2:D.n,1)=Pt (2:D.n)+G.dt*K(2:D.n)/6;
  % ------------------------------------ EE Time March --------------------------------
  K=RHS_P(D,D.P(1:D.n),G); D.P(2:D.n,1)=D.P(2:D.n)+G.dt*K(2:D.n);                
  % ------------------------- (keep one of the above 2 sections) ----------------------
  if mod(timestep,h1)==0, Rotate_Plot(D,G), title(sprintf('t = %0.5g',t)), drawnow; end
...
% --------------------------------------- 3D Box --------------------------------------
for i=round(4/G.dx):round(5/G.dx),
 for j=round(4/G.dx):round(5/G.dx),
  for k=round(4/G.dx):round(5/G.dx),
...
for l=b:D.n
  j(1:G.d)=double(D.j(l,1:G.d));
  % ----------------------------- 3D Solid Body Rotation ------------------------------
  % D.v(l,1)=2.*j(2)*G.dx; D.v(l,2)=-2.*j(1)*G.dx; D.v(l,3)=-.5;
  % ----------------------------------- 3D Lorenz -------------------------------------
  D.v(l,1)=G.sigma*(j(2)-(j(1)+0.5))*G.dx;
  D.v(l,2)=-(j(2)+0.5)*G.dx-j(1)*G.dx*j(3)*G.dx; 
  D.v(l,3)=-G.b*(j(3)+0.5)*G.dx+j(1)*G.dx*j(2)*G.dx-G.b*G.r;
  % ------------------------- (keep one of the above 2 sections) ----------------------
...
for l=2:D.n, i=D.j(l,1)+M; j=D.j(l,2)+M; k=D.j(l,3)+M;
  if i>0 & i<=N & j>0 & j<=N & k>0 & k<=N, Pfull(i,j,k)=D.P(l); end, end
clf; isosurface([-G.L:G.dx:G.L],[-G.L:G.dx:G.L],[-G.L:G.dx:G.L],Pfull,5*G.thresh);
