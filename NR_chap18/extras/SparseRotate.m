...
%%%%%%%%%%%%%%%%% begin user input %%%%%%%%%%%%%%%% 
T=3.145; G.h=0.012; G.dx=.024; G.d=2; G.thresh=0.001; G.max=3000; G.L=G.dx*127;
%%%%%%%%%%%%%%%% end of user input %%%%%%%%%%%%%%%% 
disp('Initializing'), t=0; G.Y=eye(G.d,'int16'); [D]=Initialize_D(G); h1=round(G.dx/G.h);
[D]=Modify_pointset(D,G); [D]=Modify_pointset(D,G);

disp('Plotting initial condition')
figure(1), clf, Rotate_Plot(D,G), title('Initial condition, t=0')
N=round(G.L/G.dx+1); M=round((N-1)/2+1); Pfull(1:N,1:N)=1/0;
for l=2:D.n
  i=D.j(l,1)+M; j=D.j(l,2)+M; if i>0 & i<=N & j>0 & j<=N, Pfull(i,j)=D.P(l); end
end
figure(2); clf; plot([-G.L/2:G.dx:G.L/2],Pfull(round(N/2)+1,:),'k-','Linewidth',1);
axis equal; axis([-.85 .85 0 1]); hold on;
figure(3); clf; plot([-G.L/2:G.dx:G.L/2],Pfull(:,round(1*N/4)+14),'k-','Linewidth',1);
axis equal; axis([-.42 .42 0 1]); hold on;
figure(4); clf; plot([-G.L/2:G.dx:G.L/2],Pfull(:,round(3*N/4)-16),'k-','Linewidth',1);
axis equal; axis([-.42 .42 0 1]); hold on; disp('press a key to continue...'), pause;

mu=0.0002; disp(sprintf('Plotting exact solution, t=%0.5g, mu=%0.5g',2*pi,mu))
Pfull(1:N,1:N)=0; for l=2:D.n, i=D.j(l,1)+M; j=D.j(l,2)+M;
  if i>0 & i<=N & j>0 & j<=N, Pfull(i,j)=D.P(l); end, end
L=G.L*128/127; KX=(2*pi/L)*[[0:N/2-1]']; KY=(2*pi/L)*[[0:N/2]';[-N/2+1:-1]']; % Wavenumbers
Pfullhat(:,:,1)=RFFT3D(Pfull(:,:,1),N,N,1);                  % Transform to Fourier space
for i=1:N/2, for j=1:N, PEhat(i,j)=exp(-mu*(KX(i)^2+KY(j)^2)*pi)*Pfullhat(i,j,1); end, end
PE=RFFT3Dinv(PEhat,N,N,1);                                   % Transform to Physical space
figure(1), clf; colormap(cool), surf([-.996:G.dx:.996],[-.996:G.dx:.996],PE(23:106,23:106));
axis([-1 1 -1 1 0 1.001]); title(sprintf('Exact Solution, t=%0.5g, mu=%0.5g',2*pi,mu));
figure(2); plot([-G.L/2:G.dx:G.L/2],PE(round(N/2)+1,:),'r-.','Linewidth',1);
figure(3); plot([-G.L/2:G.dx:G.L/2],PE(:,round(1*N/4)+14),'r-.','Linewidth',1);
figure(4); plot([-G.L/2:G.dx:G.L/2],PE(:,round(3*N/4)-16),'r-.','Linewidth',1);
disp('press a key to continue...'), pause;

disp(sprintf('Marching system with dt = %0.5g, dx= %0.5g',G.h,G.dx)) 
for timestep=1:T/G.h, t=t+G.h; if mod(timestep,h1)==0, [D]=Modify_pointset(D,G); end
  ...
  if mod(timestep,h1)==0, Rotate_Plot(D,G), title(sprintf('Numerical Solution, t=%0.5g',t))
end, end

disp('Plotting terminal condition')
Rotate_Plot(D,G), title(sprintf('Numerical Solution, t=%0.5g',t))
Pfull(1:N,1:N)=1/0; for l=2:D.n, i=D.j(l,1)+M; j=D.j(l,2)+M;
  if i>0 & i<=N & j>0 & j<=N, Pfull(i,j)=D.P(l); end, end
figure(2); plot([-G.L/2:G.dx:G.L/2],Pfull(round(N/2)+1,:),'b--','Linewidth',1);
figure(3); plot([-G.L/2:G.dx:G.L/2],Pfull(:,round(1*N/4)+14),'b--','Linewidth',1);
figure(4); plot([-G.L/2:G.dx:G.L/2],Pfull(:,round(3*N/4)-16),'b--','Linewidth',1); pause;
end % function SparseRotate
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
end % function Initialize_vuwik
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Rotate_Plot(D,G),
figure(1); % check_connections(D,G)
N=round(2/G.dx+1); M=round((N-1)/2+1); Pfull(1:N,1:N)=1/0;
for l=2:D.n, i=D.j(l,1)+M; j=D.j(l,2)+M;
  if i>0 & i<=N & j>0 & j<=N, Pfull(i,j)=D.P(l); end, end
colormap(cool); surf([-1:G.dx:1],[-1:G.dx:1],Pfull); axis([-1 1 -1 1 0 1.001]);
end % function Rotate_Plot
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
end, end, disp 'checked'
end % function check_connections