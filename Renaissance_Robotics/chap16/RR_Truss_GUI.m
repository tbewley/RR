...
dd = uidropdown(g,"ValueChangedFcn",@(src,event)update1(src,event), ...
    "Items",["Warren","Howe","Pratt","Pennsylvania","K","Modified K"]);
...
switch truss                                   
    ...
    case 'Pennsylvania'
        ...
        for i=2:s-1, Q(:,q+i)=[(i-1/2)/s; min(Q(2,sb+i-2),Q(2,sb+i-1))/2]; end; q=q+s-2; % free nodes in the middle row
...
switch truss                        % free nodes in top row
    ...
    case 'Pratt'
        ...
        C(j+1,n-2)=-1; C(j+1,n)=1; j=j+1;                           % right diagonal to fixed node
    case 'Pennsylvania'
        m=sb+3*s-3; C=zeros(m,q+p);      % FIX THIS.
        C(1,n-1)=-1;   C(1,1)=1; j=1;                                % bottom row to left fixed node
