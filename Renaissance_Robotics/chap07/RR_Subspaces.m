...
clc, A=[2 -2 -4;-1 3 4;1 -2 -3], [C,L,R,N,Apinv]=test_3_by_3_matrix(A);
disp('here is a surprise! (A is idempotent)');
A, A_times_A=A*A, pause, disp(' ')

clc, A=[2 2 -2;5 1 -3;1 5 -3], [C,L,R,N,Apinv]=test_3_by_3_matrix(A);
...
disp('is to calculate [U,S,V] = svd(A), as described in the notes.')

function [C,L,R,N,Apinv]=test_3_by_3_matrix(A)
...
function [C,L,R,N,Apinv]=test_rectangular_matrix(A)
r=rank(A), pause, disp(' ');
[C,L]=QRcheck(A,r),  pause, disp(' ')
[R,N]=QRcheck(A',r), pause, disp(' ')
Apinv=pinv(A), fac=scale_factor(Apinv), Apinv_times_fac=Apinv*fac, pause, disp(' ')
