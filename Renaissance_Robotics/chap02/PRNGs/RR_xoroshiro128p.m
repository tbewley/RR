function [out,s]=RR_xoroshiro128p(n,s)
% function [out,s]=RR_xoroshiro128p(n,s)
% PRNG using Blackman & Vigna's xoroshiro128+ scheme (note: n,s are both required inputs)
% output computed with          z←(s0+s1)>>11
% PRNG marched forward with     s1←s1^s0,  s0←(s0<<<24)^s1^(s1<<16),  s1←(s1<<<37) 
...
% See https://prng.di.unimi.it/xoroshiro128plus.c for Blackman & Vigna's C implementation.
%
...
for i=1:n
  out(i)=bitsrl(RR_sum64(s(1),s(2)),11);                        % one addition
