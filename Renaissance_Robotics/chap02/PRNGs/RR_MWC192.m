function [out,s]=RR_MWC192(n,s)
% function [out,s]=RR_MWC192(n,s)
% PRNG using Vigna's tuning of Marsaglia's MWC192 scheme (n,s are both required inputs)
%      out=y, t←x*a+c, x←y, [c;y]←t
% NOTE: {x,y,c} denoted {s(1),s(2),s(3)} in code below
% See RR_MWC for a convenient wrapper that takes care of state initialization. 
% See https://prng.di.unimi.it/MWC192.c for Vigna's C implementation.
%
% INPUT:        n = number of pseudorandom numbers to generate
% INPUT/OUTPUT: s = vector of three uint64 state variables {x,y,c}
