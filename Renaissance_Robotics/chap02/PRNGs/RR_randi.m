...
% function X=RR_randi(IMAX,M,N,P) or X=RR_randi(IMAX,[M N P]) or X=RR_randi(size(A))
% Pseudorandomly generate a scalar, vector (length M), matrix (size MxN), or rank-3 array (size MxNxP),
% each entry of which is a integer with discrete uniformly distribution on 1:IMAX
...
% Then, select a random uint32 integer on [0,draw_max] using RR_PCG32, normalize X by
% cat_total to generate integers on 1:IMAX, and reshape into the desired matrix form
