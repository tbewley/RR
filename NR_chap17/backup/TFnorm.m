function x=RR_TFnorm(A,B,C,p,MODE)
% function x=RR_TFnorm(A,B,C,[p],[MODE])
...
gamma=10^x;
if P.MODE=='CT'
  lambda=RR_Eig([P.A P.S/gamma; -P.Q/gamma -P.A'],'r');
  f=-1; for i=1:P.n*2, if abs(real(lambda(i)))<1e-5, f=1; return, end, end
else
  lambda=RR_Eig([P.A-P.S*P.Ai*P.Q/gamma^2 P.S*P.Ai/gamma; -P.Ai*P.Q/gamma P.Ai],'r');
