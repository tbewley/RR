...
% EXAMPLE: (see examples for each special case in the code below)
...
switch func
  case 'tan'
    ...
    % EXAMPLE: in=1.0; RR_cordic_derived('tan',in,40,cordic_tables), tan(in)
    v=RR_cordic('cos_sin',in,n,cordic_tables);
    out=v(2)/v(1);
  case 'tanh'
    % out=tanh(in)
    % uses identity tanh(x)=sinh(x)/cosh(x)
    % EXAMPLE: in=1.0; RR_cordic_derived('tanh',in,40,cordic_tables), tanh(in)
    v=RR_cordic('cosh_sinh',in,n,cordic_tables);
    out=v(2)/v(1);
  case 'exp'
    % out=exp(in)
    % uses identity exp(x)=sinh(x)+cosh(x)
    % EXAMPLE: in=1.0; RR_cordic_derived('exp',in,40,cordic_tables), exp(in)
    v=RR_cordic('cosh_sinh',in,n,cordic_tables);
    out=v(1)+v(2);
  case 'log' 
    % out=log(in)  where log denotes the natural log, aka ln
    % uses identity log(x)=2*atanh((w-1)/(w+1))
    % EXAMPLE: in=1.2; RR_cordic_derived('log',in,40,cordic_tables), log(in)
