% script RR_attitude_with_Jacobian.m
%
...
%
% This code accelerates RR_attitude by implementing an analytical Jacobian.
%
...
% as well as the Jacobian, J, of this error.
...
J=[2*q2*gz-2*q3*gy+2*q0*gx, 2*q3*gz+2*q2*gy+2*q1*gx, 2*q0*gz+2*q1*gy-2*q2*gx,2*q1*gz-2*q0*gy-2*q3*gx;...
  -2*q1*gz+2*q0*gy+2*q3*gx,-2*q0*gz-2*q1*gy+2*q2*gx, 2*q3*gz+2*q2*gy+2*q1*gx,2*q2*gz-2*q3*gy+2*q0*gx;...
   2*q0*gz+2*q1*gy-2*q2*gx,-2*q1*gz+2*q0*gy+2*q3*gx,-2*q2*gz+2*q3*gy-2*q0*gx,2*q3*gz+2*q2*gy+2*q1*gx;...
   2*q2*mz-2*q3*my+2*q0*mx, 2*q3*mz+2*q2*my+2*q1*mx, 2*q0*mz+2*q1*my-2*q2*mx,2*q1*mz-2*q0*my-2*q3*mx;...
  -2*q1*mz+2*q0*my+2*q3*mx,-2*q0*mz-2*q1*my+2*q2*mx, 2*q3*mz+2*q2*my+2*q1*mx,2*q2*mz-2*q3*my+2*q0*mx;...
   2*q0*mz+2*q1*my-2*q2*mx,-2*q1*mz+2*q0*my+2*q3*mx,-2*q2*mz+2*q3*my-2*q0*mx,2*q3*mz+2*q2*my+2*q1*mx;...
   2*q0,                    2*q1,                    2*q2,                   2*q3                   ];
end

function [F,J]=func2(x)
global g gr m mr
% This code determines the error F corresponding to the fit between the
% unrotated magnetic and gravity vectors, {m,g}, and the rotated magnetic
% and gravity vectors, {mr,gr}, due to rotation by a unit quaternion q,
% as well as the Jacobian, J, of this error,
% ASSUMING that g=(0,0,1) and m=(1,0,0) [thus, simplifying computations].
% Driving F to zero using the levenberg-marquardt algoritm implemented in
% fsolve thus optimizes the rotation matrix R generated by this unit
% quaternion q in order to recover both gr=R*g and mr=R*m.
q0=x(1); q1=x(2); q2=x(3); q3=x(4);
R_times_g=[ 2*q1*q3 + 2*q0*q2;
            2*q2*q3 - 2*q0*q1;
            q3^2-q2^2-q1^2+q0^2];
R_times_m=[-q3^2-q2^2+q1^2+q0^2;
            2*q1*q2 + 2*q0*q3;
            2*q1*q3 - 2*q0*q2];
F(1:3)= R_times_g - gr(1:3);
F(4:6)= R_times_m - mr(1:3);
F(7)= q0^2+q1^2+q2^2+q3^2 - 1;
