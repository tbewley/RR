...
%          testg=RR_PolyAdd(RR_PolyProd(a,xg),RR_PolyProd(b,yg)); residualg=norm(RR_PolyAdd(g,-testg))
...
x=0; y=1; for j=n-1:-1:1                           % Using the q{i} from the GCF call, compute {xg,yg}
  t=x; x=y; y=RR_PolyAdd(t,-RR_PolyProd(q{j},y));  % via the Extended Euclidean algorithm
