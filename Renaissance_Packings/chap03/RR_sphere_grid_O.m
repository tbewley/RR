...
clear, n=3  % n=0,1,2,3,4,... degree of refinement of each edge (N=2^n intervals)
...
[x(:,:,:,1),xR(:,:,:,1)]=RR_sphere_gen_orthant(N,omega,1,A,C,D);
[x(:,:,:,2),xR(:,:,:,2)]=RR_sphere_gen_orthant(N,omega,1,A,E,F);
[x(:,:,:,3),xR(:,:,:,3)]=RR_sphere_gen_orthant(N,omega,2,A,D,E);
[x(:,:,:,4),xR(:,:,:,4)]=RR_sphere_gen_orthant(N,omega,2,A,F,C);
[x(:,:,:,5),xR(:,:,:,5)]=RR_sphere_gen_orthant(N,omega,2,B,C,D);
[x(:,:,:,6),xR(:,:,:,6)]=RR_sphere_gen_orthant(N,omega,2,B,E,F);
[x(:,:,:,7),xR(:,:,:,7)]=RR_sphere_gen_orthant(N,omega,1,B,D,E);
[x(:,:,:,8),xR(:,:,:,8)]=RR_sphere_gen_orthant(N,omega,1,B,F,C);
view(57,19)
