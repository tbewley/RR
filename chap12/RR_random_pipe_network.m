function [X,node,n,tri]=RR_random_pipe_network

n=30; L=11; X=[randi(L,n-1,2); 1 1]-[1 1]; X=sortrows(sortrows(X,2));
while 1; [X,n,modified]=remove_duplicates(X,n); if ~modified, break, end, end, n
tri = delaunay(X)
close all, figure(1), triplot(tri,X(:,1),X(:,2)), hold on, pause
for i=1:n
...
X, node(:).connected, node(:).length, node(:).num_pipes
