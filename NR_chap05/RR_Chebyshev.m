function [x]=RR_Chebyshev(n,x,derivative,kind)
...
switch kind
  case 1, switch derivative
    ...
    case 1, x=n*RR_Chebyshev(n-1,x,0,2);
    case 2, switch x
       case  1, x=(n^4-n^2)/3;
       case -1, x=(-1)^n*(n^4-n^2)/3;
       otherwise, x=n*((n+1)*RR_Chebyshev(n,x,0,1)-RR_Chebyshev(n,x,0,2))/(x^2-1); end
  ...
  case 2, switch derivative
    ...
    case 1, x=((n+1)*RR_Chebyshev(n+1,x,0,1)-x*RR_Chebyshev(n,x,0,2))/(s^2-1);
    otherwise, x=0; disp('Case not yet implemented'); end
end
