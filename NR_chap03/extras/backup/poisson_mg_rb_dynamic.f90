program RR_poisson_mg_rb             ! A 2D Poisson solver on uniform mesh using Multigrid
implicit none                     ! and Red/Black RR_Gauss-Seidel (Fortran90 syntax).
...
do smooth=1,n1; call RR_poisson_rb(v,d,g,g%xm,g%ym); end do
call date_and_time(values=ta); t = ta(5)*3600 + ta(6)*60 + ta(7) + 0.001*ta(8)
main: do i=1,15
   o=e; call RR_poisson_mg(v,d,g,g%xm,g%ym,n2,n3); e = max_error(v,d,g,g%xm,g%ym)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
recursive subroutine RR_poisson_mg(vf,df,gf,gfxm,gfym,n2,n3)
...
do smooth=1,n2; call RR_poisson_rb(vf,df,gf,gf%xm,gf%ym); end do 
...
if (gc%nx > 3 .and. gc%ny > 3 .and. mod(gf%nx,4)==0 .and. mod(gf%ny,4)==0) then
   call RR_poisson_mg(vc,dc,gc,gc%xm,gc%ym,n2,n3)                   ! Continue to even
else                                                             ! coarser grid, or
   do smooth=1,20; call RR_poisson_rb(vc,dc,gc,gc%xm,gc%ym); end do ! solve coarsest system
end if
...
do smooth=1,n3; call RR_poisson_rb(vf,df,gf,gf%xm,gf%ym); end do  
end subroutine RR_poisson_mg
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine RR_poisson_rb(v,d,g,gxm,gym)
