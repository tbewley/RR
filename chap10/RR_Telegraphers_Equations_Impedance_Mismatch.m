...
Rr=0; Gr=0; Lr=L; Cr=4*C; Z0r=sqrt(Lr/Cr), cr=1/sqrt(Lr*Cr),  % CHANGE L and C IN RIGHT HALF OF WIRE
for i=N/2+1:N,
...
if termination, A(2*N,2*N)=A(2*N,2*N)-(d/C)/Z0r; end
...
for n=1:n_max
   ...
   x=D\r; t=n*h;
   if termination, Iright=x(2*N)/Z0r; else, Iright=0; end
   if mod(n,50)==0, subplot(2,1,1), plot(XV, [Vleft; x(2:2:2*N)]), axis([0 X -0.2 4.2]), 
                    ...
                    title(['Current, t/T = ',num2str(t/T)]), hold on, pause(0.4), clf, end
 end
