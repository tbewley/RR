...
alpha=1; m=1; a=sqrt(2*alpha*9.8); numG=[1/m]; denG=Conv([1 sqrt(-1)*a],[1 -sqrt(-1)*a]);
...
Nyquist(Conv(numG,numD),Conv(denG,denD)),
...
K=98; z=4.49; p=8; numD=K*[1 0 z^2]; denD=Conv([1 p],[1 p]); g.linestyle='b--';
TestSystem(numG,denG,numD,denD,g)      
Nyquist(Conv(numG,numD),Conv(denG,denD)), figure(6), print -depsc cartoscStableNyquist.eps
...
K=98; z=4.37; p=8; numD=K*[1 0 z^2]; denD=Conv([1 p],[1 p]); g.linestyle='r-'; g.T=50;
