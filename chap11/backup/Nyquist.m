...
% function Nyquist(num,den,g)
% Plot a Nyquist plot (i.e., a Bode plot in polar coordinates) of the plant D(s)*G(s)=
% num(s)/den(s).  The derived type g groups together convenient plotting parameters:
% g.K is the set of gains used, {g.figs,g.figL} are the figure numbers, and, in the
% s plane, g.eps is the (small) radius of the half-circles to the right of each pole
% on the imaginary axis, and g.R is the (large) radius of the D contour.
