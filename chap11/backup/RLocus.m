...
% G(s)=numG(s)/denD(s).  The derived type g groups together convenient plotting parameters:
...
numGD=PolyConv(numG,numD); denGD=PolyConv(denG,denD); MS='MarkerSize'; clf, hold on
for j=1:length(g.K); denH=PolyAdd(g.K(j)*numGD,denGD,'r');
  Hpoles=roots(denH); plot(real(Hpoles),imag(Hpoles),'k.',MS,6)
end, 
...
Hpoles=roots(PolyAdd(numGD,denGD,'r')); plot(real(Hpoles),imag(Hpoles),'r*',MS,17)
