function NyquistDT(num,den,g,h)
% function NyquistDT(num,den,g,h)
% Draw the Nyquist plot (i.e., a Bode plot in polar coordinates) of G(z)=num(z)/den(z).
% The derived type g groups together convenient plotting parameters: g.omega is the set of
% frequencies used, g.style is the linestyle, {g.figz,g.figL} are the figure numbers, and,
% in the z plane, g.eps is the (small) radius of the half-circles outside each pole
% on the unit circle, and g.R is the (large) radius of the D contour.
% Recommendation: don't make g.eps too small, or g.R too big, until you see where the
% corresponding curves are in both the s plane and the L plane!
...
figure(g.figz), clf, plot(real(P),imag(P),'kx'), hold on, plot(real(Z),imag(Z),'ko')
...
figure(g.figz), plot(real(w),imag(w),sym), figure(g.figL), plot(real(L),imag(L),sym)
end
