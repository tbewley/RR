function [x,t,s]=RR_BDF3iter(R,x,t,s,p,v,SimPlot)
% function [x,t,s]=RR_BDF3iter(R,x,t,s,p,v,SimPlot)
% Simulate x'=f(x), with f implmented in R, using the iterative BDF3 method with an eBDF3
% predictor.  t contains the initial t on input and the final t on output. x contains
% the 3 (or more) most recent values of x on input (from a call to RR_BDF2iter/RR_BDF3iter),
% and the 4 most recent values of x on output (facilitating a call to RR_BDF3iter/RR_BDF4iter).
...
% See also RR_IEiter, RR_BDF2iter, RR_BDF4iter, RR_BDF5iter, RR_BDF6iter.  Verify with RR_BDFtest.
