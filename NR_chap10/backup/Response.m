function RR_Response(A,B,C,D,type,g)
% function RR_Response(A,B,C,D,type,g)
...
for m=1:M
  ...
  [x(:,2),Amod]=RR_Gauss(Am,Ap*x(:,1)+Bs*(u(2)+u(1)),N);
  for k=3:km+1, [x(:,k)]=RR_GaussLU(Amod,Ap*x(:,k-1)+Bs*(u(k)+u(k-1)),N); end
  for p=1:P, subplot(P,M,m+p*P-P), plot(t,C(p,:)*x(:,[0:km]+1)+D(p,m)*u,g.linestyle), end
end
end % function RR_Response
