function FS_Relax(x,verbose,m)                       % Numerical Renaissance Codebase 1.0
...
plot((f(2:n)-f(1:n-1))/h,[0:h:etamax-h],'r--');
...
for i=1:20                                                  % Start the iteration.
  ...
  f=Penta(a,b,c,d,e,x,n); plot((f(2:n)-f(1:n-1))/h,[0:h:etamax-h],'b-.');  % Do the solve.
end
plot((f(2:n)-f(1:n-1))/h,[0:h:etamax-h],'k-');  print -depsc FSrelax.eps
% end function FS_Relax.m