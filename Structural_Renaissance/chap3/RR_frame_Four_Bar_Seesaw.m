...
S.P=[ 0  0;   % The columns of {Q,P} denote the x,y locations of each of the {free,pinned} nodes
      0 2*h]; % There are q=10 free nodes and p=2 pinned support nodes in this structure
...
figure(1); RR_Structure_Plot(S,L,x); x_error=norm(A*x-b)

% Also plot the direction of the nullspace vector
L.U=zeros(2,10); xn=null(A); figure(2); RR_Structure_Plot(S,L,xn); xn_error=norm(A*xn)
