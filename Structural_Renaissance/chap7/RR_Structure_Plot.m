...
if S.d==2               % This handles the rest of the 2D case
  ...
  if S.p+S.r+S.s>0, fprintf('Sum of all reaction forces in (x,y) = (%+0.5g, %+0.5g) N\n',t2(1),t2(2)), end
  for i=1:S.q
    if L.U_in(i)
      f=quiver(N(1,i)-fac_f*L.U(1,i),N(2,i)-fac_f*L.U(2,i),fac_f*L.U(1,i),fac_f*L.U(2,i),0);
    else
      f=quiver(N(1,i),N(2,i),fac_f*L.U(1,i),fac_f*L.U(2,i),0);
    end
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','m');
  end
  for i=1:S.p
    if S.P_in(i)
       f=quiver(S.P(1,i)-fac_f*VP(1,i),S.P(2,i)-fac_f*VP(2,i),fac_f*VP(1,i),fac_f*VP(2,i),0);
    else
       f=quiver(S.P(1,i),S.P(2,i),fac_f*VP(1,i),fac_f*VP(2,i),0);
    end
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','r');
  end
  for i=1:S.r
    if S.R_in(i)
       f=quiver(S.R(1,i)-fac_f*VR(1,i),S.R(2,i)-fac_f*VR(2,i),fac_f*VR(1,i),fac_f*VR(2,i),0);
    else
       f=quiver(S.R(1,i),S.R(2,i),fac_f*VR(1,i),fac_f*VR(2,i),0);
    end
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','r');
  end
  for i=1:S.s
    if S.R_in(i)
       f=quiver(S.S(1,i)-fac_f*VS(1,i),S.S(2,i)-fac_f*VS(2,i),fac_f*VS(1,i),fac_f*VS(2,i),0);
    else
       f=quiver(S.S(1,i),S.S(2,i),fac_f*VS(1,i),fac_f*VS(2,i),0);
    end
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','r');
    if MS(i)<0
      r=0.4; t=-2*pi*7/8; x=S.S(1,i)+r*cos(t); y=S.S(1,i)+r*sin(t);
      drawcircle(S.S(:,i),0.4,3,'r',0,t)
  ...
  if S.p+S.r+S.s>0, fprintf('Sum of all reaction forces in (x,y,z) = (%+0.5g, %+0.5g, %+0.5g) N\n',t2(1),t2(2),t2(3)), end
  fac_b=fac_b*0.07; fac_f=fac_f*2;                   % Tweak the scale factors for 3D
  for i=1:S.q
    if L.U_in(i)
        f=quiver3(S.Q(1,i)-fac_f*L.U(1,i), S.Q(2,i)-fac_f*L.U(2,i), S.Q(3,i)-fac_f*L.U(3,i), ...
                           fac_f*L.U(1,i),          fac_f*L.U(2,i),          fac_f*L.U(3,i),0);
    else
        f=quiver3(S.Q(1,i),       S.Q(2,i),       S.Q(3,i), ...
                  fac_f*L.U(1,i), fac_f*L.U(2,i), fac_f*L.U(3,i),0);
    end
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','m');
  end
  for i=1:S.p
    if S.P_in(i)
      f=quiver3(S.P(1,i)-fac_f*VP(1,i), S.P(2,i)-fac_f*VP(2,i), S.P(3,i)-fac_f*VP(3,i), ...
                         fac_f*VP(1,i),          fac_f*VP(2,i),          fac_f*VP(3,i),0);
    else
      f=quiver3(S.P(1,i),      S.P(2,i),      S.P(3,i), ...
                fac_f*VP(1,i), fac_f*VP(2,i), fac_f*VP(3,i),0);
    end      
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','r');
  end
  for i=1:S.r
    if S.R_in(i)
      f=quiver3(S.R(1,i)-fac_f*VR(1,i), S.R(2,i)-fac_f*VR(2,i), S.R(3,i)-fac_f*VR(3,i), ...
                         fac_f*VR(1,i),          fac_f*VR(2,i),          fac_f*VR(3,i),0);
    else
      f=quiver3(S.R(1,i),      S.R(2,i),      S.R(3,i), ...
                fac_f*VR(1,i), fac_f*VR(2,i), fac_f*VR(3,i),0);
    end
    set(f,'MaxHeadSize',10000,'linewidth',3,'color','r');
  end
  for i=1:S.s
    if S.S_in(i)
      f=quiver3(S.S(1,i)-fac_f*VS(1,i), S.S(2,i)-fac_f*VS(2,i), S.S(3,i)-fac_f*VS(3,i), ...
